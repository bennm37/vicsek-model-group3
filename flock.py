...
class Flock():
    ...
    def animate_movement(self,frames,sigma,type="vicsek",args={},plot_order_stat=False):
        ...
        if plot_order_stat:
            ...
            q =ax1.quiver(init_pos[:,0],init_pos[:,1],init_dir[:,0],init_dir[:,1],scale = 70,color="b")
...
class Flock_3d():
    ...
    def animate_movement(self,sigma,num_frames,ambient_rotation=False):
        ...
        ax.set_title('3D Vicsek Birds')

        p =self.positions
        d = self.directions
        graph = ax.scatter(p[:,0],p[:,1],p[:,2],color="k",marker="o")
...
class Prey(Flock):
    ...
    def update_posdirs(self,sigma,predator,repulsion_factor =1,verbose=False):
...
class Moth(Flock):
    def update_posdirs(self,sigma,leader,attraction_factor =1,verbose=False):
...
class Predator(Flock):
    ...
    def update_posdirs(self,sigma,prey,attraction_factor =1,verbose=False):
        dt=1
        ##update via vicsek equations,moving back over the frame if it crosses a boundary
        new_positions = self.positions + self.get_directions()*self.speed*dt
        new_positions = new_positions %self.frame_size
    ...
    def animate_movement_pp(self,prey,frames,sigma,interval,repulsion_factor=1,attraction_factor=1):
        ...
        def animate(i):
            ...
            prey.update_posdirs(sigma,self.positions)
            self.update_posdirs(sigma,prey.positions)
